...
- **57 项单元测试**全通过，覆盖解析 / 烈度衰减 / 多源融合 / 跨源去重等核心逻辑

---

## 核心架构

Dianguard 采用"**双实时源 + 多源融合 + 分区烈度衰减 + 渐进式响应**"四层架构，底层物理模型基于 GB 18306-2015《中国地震动参数区划图》，上层决策由自研多源融合引擎驱动。
...
实时数据源（域名完全独立，双路冗余）
  ...
  解析层：parseEew / parseExternalEew / parsePodrisEew / parseIclEew
        │
        ▼
  自研多源融合决策引擎 ── 多源交叉验证 · 1.5s 融合窗 · 抗异常值融合
        │
        ▼
...
> ⚠️ **单点风险治理**：v1.3.0 已移除 BeeCLD·2v8（令牌改由用户自行填写）与 FAN Studio（需 API 密钥）。当前 Wolfx WebSocket + ICL HTTP 轮询**域名完全独立**，任一可达即触发，双源报文共同进入多源融合引擎交叉验证。
...
自研多源融合决策引擎在 **1.5s 融合窗口**内收集多源报文，通过**多源交叉验证 + 抗异常值融合**降低单源误报风险，并具备坐标防抖、按事件版本隔离、超时强制输出等工程保障。

> ℹ️ 本开源仓库包含融合引擎的**兼容精简实现**（加权融合 + 坐标选优）；完整版引擎为**闭源专有**（详见文末《许可声明》），不随本仓库分发。
...
```bash
./gradlew testDebugUnitTest   # 57 个用例，覆盖解析/烈度/融合/去重
...
- **闭源部分**：**多源融合决策引擎及其自学习校准模块**（源码目录中的 `EewFusion.kt` 完整版、`FusionCalibration.kt` 等，以及随版本迭代演进的对应实现）为**闭源专有**，**不随本仓库分发**，其完整实现仅由作者维护。本仓库中同名文件的**开源精简兼容实现**仅保留对外接口（`ingest` / `forceFlush` / `FusionResult`），内部算法为简化加权融合，**不构成**闭源引擎的授权或替代。
