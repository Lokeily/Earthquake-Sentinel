...
class EewConnectionManager(val service: EewService) {
    ...
    private fun makeListener(sourceId: String) = object : WebSocketListener() {
        ...
        override fun onFailure(webSocket: WebSocket, t: Throwable, response: Response?) {
            ...
            if (c != null) {
                c.failCount++
                patchSourceState(sourceId) { copy(connected = false, failCount = c.failCount) }
                if (c.failCount == EewService.CONN_FAIL_WARN_THRESHOLD) {
                    val name = connections[sourceId]?.source?.name ?: sourceId
                    Log.w(EewService.TAG, "数据源 $name 连续失败 ${c.failCount} 次，可能已不可用，将持续退避重试")
                }
                c.scheduleReconnect()
            } else {
        ...
        override fun onClosed(webSocket: WebSocket, code: Int, reason: String) {
            ...
            EewService.connectedSourceCount = connectedSources.size
            patchSourceState(sourceId) { copy(connected = false) }
            connections[sourceId]?.scheduleReconnect()
            refreshHeadline()
            if (connectedSources.size == 0) scheduleAllDownCheck()
        }
    }
